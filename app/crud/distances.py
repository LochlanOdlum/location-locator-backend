...
def create_location_distances(db: Session, location: LocationRead, ors_client: OpenRouteServiceClient):
  for home in db.query(Home).all():
    ...
    # Capture ValueErrors when no distance is found, as depending on user locations distances are sometimes not easily calculable
      ...
      print(f"Failed to get distance between home id: {home.id} and location {location.id}")
...
def update_home_distances(db: Session, home: HomeRead, ors_client: OpenRouteServiceClient):
...
def update_location_distances(db: Session, location: LocationRead, ors_client: OpenRouteServiceClient):
